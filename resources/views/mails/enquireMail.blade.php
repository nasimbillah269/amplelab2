...
<link rel="shortcut icon" href="{{assetUrl(general()->favicon())}}">
...
<center>
<img src="{{assetUrl(general()->logo())}}" style="max-width:200px;">
<h2 style="margin:0;">{{$datas['r']->name}}</h2>
<p style="margin:0;">Welcome To {{general()->title}} !</p>
</center>

<div style="background:#f1f1f1;padding:10px;margin: 10px 0;">
  <p style="margin: 0;border-bottom: 1px solid #c6c6c6;padding: 5px 0;">PRODUCT REQUEST FORM CUSTOMER</p>
  <br>
  ...
  <p style="margin:0;"><strong>Message:</strong><br> {{$datas['r']->message}}</p>
  <br>
...
<div style="background:#f1f1f1;padding:10px;margin: 10px 0;">
  ...
  <p>{!!general()->address_one!!} <br><strong>Email: </strong> {{general()->email}}<br> <strong>Mobile: </strong> {{general()->mobile}} <br><a href="{{general()->website}}" target="_blank"> {{general()->website}}</a> </p>
</div>
