@foreach($product->productAttibutesVariationGroup() as $ii=>$attri)
    <div class="row" style="margin:0 -10px;">
        <div class="col-md-12" style="padding:10px;">
            <h5 style="margin-bottom: 5px;font-weight: bold;"> {{$product->productVariationAttributeList()->where('parent_id',$attri->id)->count()}} {{$attri->name}} <span class="selected-value text-success"></span></h5>
        </div>
            ...
            <ul class="colorList attributeItem">
                ...
                <li>
                 ...
                 data-bs-placement="top" data-bs-title="{{$attri->name}}"
                	...
                	@php
                        $imgs = App\Models\PostAttribute::where('src_id', $product->id)
                                    ->where('reff_id', 68)
                                    ->where('parent_id', $sku->attribute_item_id)
                                    ->first();
                    
                        $imagesArray = [];
                        if ($imgs && $imgs->galleryFiles) {
                            foreach ($imgs->galleryFiles as $file1) {
                                $imagesArray[] = assetUrl($file1->image());
                            }
                        }
                    @endphp
                    ...
                    data-image="{{assetUrl($sku->variationItemImage($product->id))}}"
                    @endif
                    ...
                    @endif
                	for="for_{{$attri->name}}_{{$k+1}}"
                	><img src="{{assetUrl($sku->attributeItemValue())}}" /> </label>
                	@else
