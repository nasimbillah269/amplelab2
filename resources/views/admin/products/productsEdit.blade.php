                    ...
                    <div class="card">
                        ...
                        <div class="card-content">
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    <img src="{{assetUrl($product->image())}}" style="max-width: 100px;" />
                                    @isset(json_decode(Auth::user()->permission->permission, true)['products']['add'])
                                ...
                                <div class="row">
                                    ...
                                    <img src="{{assetUrl($gallery->file_url)}}" style="max-width: 60px;max-height: 60px" />
                                ...
                                <div class="form-group">
                                    <img src="{{assetUrl($product->banner())}}" style="max-width: 100px;" />
                                    @isset(json_decode(Auth::user()->permission->permission, true)['products']['add'])
...
<script>
    ...
    $(document).ready(function(){
        ...
        ///Product Price Jquery
            ...
            $(document).on('change', '.changeImage2', function() {
                let $input = $(this);
                let valueId = $input.data('id');
                let uploadUrl = $input.data('url');
                let $imagesContainer = $input.closest('.input-group').next('.variantImages'); 
                ...
                formData.append('variation_image', files[0]);
                        ...
                        if (response.success) {
                            let newImageHtml = `
                                <span style="display:inline-block; margin: 2px;">
                                    <img src="${response.image_url}" style="height: 40px; max-width: 100%;">
                                    <a href="${response.delete_url}" class="mediaDelete2" style="color: red;">
                                        <i class="fa fa-trash"></i>
                                    </a>
                                </span>
                            `;
                            $imagesContainer.append(newImageHtml);
                        } else {
            ...
            $(document).on('click', '.mediaDelete2', function(e) {
                e.preventDefault();
                let $btn = $(this);
                ...
                if (confirm('Are you sure you want to delete this image?')) {
                    $.ajax({
                        ...
                        type: 'GET',
                        success: function(response) {
                            $btn.closest('span').fadeOut(300, function() {
                                $(this).remove();
                            });
                        },
                        error: function() {
