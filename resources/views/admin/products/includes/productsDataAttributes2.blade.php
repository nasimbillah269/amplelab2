        ...
        <table class="table table-bordered">
            ...
            <tr>
                ...
                <td style="vertical-align: middle;padding:5px;text-align:center;">
                    @php
                        $hasImage=null;
                        if($item =$variationItem->attributeVatiationItems()->where('attribute_id',68)->first()){
                            $hasImage =App\Models\PostAttribute::where('src_id',$item->product_id)->where('type',9)->where('reff_id',68)->where('parent_id',$item->attribute_item_id)->first();
                        }
                    @endphp
                    @if($hasImage)
                    
                    @foreach($hasImage->galleryFiles as $file1)
                    <span style="display:inline-block;">
                        <img src="{{assetUrl($file1->image())}}" style="max-width:100px;max-height:40px;">
                    </span>
                    @endforeach
                    @else
                    
                    @endif
                    <!--<img src="{{assetUrl($variationItem->variationItemImage())}}" style="max-width:100px;max-height:40px;">-->
                </td>
                @foreach($product->productVariationAttibutes as $att)
                            ...
                            <table class="table table-bordered">
                                ...
                                <tr>
                                    ...
                                    <td style="padding:2px;">
                                        <div class="input-group">
                                            <input type="file" name="variation_image_{{$attriValue->id}}" class="form-control changeImage2" data-id="{{$attriValue->id}}" data-url="{{route('admin.productsUpdateAjax',['attributesVariationImageUpload',$product->id])}}" data-image=".variaValueImage_{{$attriValue->id}}" accept="image/*" >
                                        </div>
                                        <div class="variantImages" style="text-align: center;">
                                            @foreach($attriValue->galleryFiles as $file)
                                            <span style="display:inline-block;">
                                                <img src="{{assetUrl($file->image())}}" style="height: 40px;max-width: 100%;">
                                                <a href="javascript:void(0)" data-url="{{route('admin.mediesDelete',$file->id)}}" class="mediaDelete2" style="color: red;"><i class="fa fa-trash"></i></a>
                                            </span>
                                            @endforeach
                                        </div>
                                    </td>
