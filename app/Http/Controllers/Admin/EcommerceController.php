...
class EcommerceController extends Controller
{
    ...
    public function productsUpdateAjax(Request $r,$column,$id){
      ...
      if($r->ajax() && $product){
        ...
        if($column=='attributesVariationImageUpload'){
            
            $valueId = $r->value_id;
            if ($r->hasFile('variation_image')){
                $file = $r->file('variation_image');
                $src = $valueId;
                $srcType = 8;
                $fileUse = 3;
                $fileStatus=false;
                $author=Auth::id();
                $media = uploadFile($file, $src, $srcType, $fileUse,$author,$fileStatus); 
    
                if($media){
                    return response()->json([
                        'success' => true,
                        'message' => 'Image uploaded successfully!',
                        'image_url' => assetUrl($media->image()),
                        'delete_url' => route('admin.mediesDelete', $media->id)
                    ]);
                }
            }
